...
task_dev() { # Start the development environment
  ...
  echo Astro port: "$astro_dev_port"
  local rr_dev_port="$(ip_random_free_port)"
  echo React Router port: "$rr_dev_port"
  ...
  INVOCATION_MODE=background task_rr__dev </dev/null
...
task_claude_md__update() { # Update `CLAUDE.md`
  mdpp --in-place CLAUDE.md
}
